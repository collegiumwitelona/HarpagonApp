...
const SettingsPage = () => {
  ...
  const handleChangePassword = async (event) => {
    ...
    if (!normalizedPreviousPassword || !normalizedNewPassword || !normalizedConfirmPassword) {
      setChangePasswordError(t('settings.changePasswordValidationError'));
      return;
    }
    ...
    try {
      const response = await api.post(
        ...
        {
          previousPassword: normalizedPreviousPassword,
      ...
      if (response.status < 200 || response.status >= 300) {
        ...
        const apiErrors = Array.isArray(response?.data?.errors)
          ...
          : [];
        const fallbackError = t('settings.changePasswordError');
        setChangePasswordError(apiMessage || apiErrors.join(' ') || fallbackError);
        return;
      }
      ...
      setChangePasswordError(t('settings.changePasswordError'));
    } finally {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-[2px]">
          <div className="max-w-md w-full bg-white rounded-[2.5rem] p-8 shadow-2xl border border-slate-100">
            ...
            <form onSubmit={handleChangePassword} className="space-y-5 text-left">
              ...
              {changePasswordError && (
                <div className="bg-rose-50 border border-rose-100 rounded-xl py-3 px-4">
                  <p className="text-rose-500 text-sm text-center font-medium">{changePasswordError}</p>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                ...
                <button
                  ...
                  className="w-full bg-violet-700 text-white font-bold py-3 rounded-2xl hover:bg-violet-800 transition-colors shadow-lg shadow-violet-200 disabled:bg-slate-300"
                >
                  {passwordActionLoading ? t('auth.changingPassword') : t('auth.changePassword')}
                </button>
              </div>
            </form>
          </div>
