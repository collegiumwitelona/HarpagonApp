...
const TransactionForm = ({ accountId, categories = [], onTransactionAdded }) => {
  ...
  const form = useForm(
    ...
    async (values) => {
      ...
      if (!accountId || !selectedCategoryId) {
        form.setError(t('history.missingAccountOrCategory'));
        return;
      }
      ...
      try {
        ...
        form.setError(t('history.addTransactionError'));
      }
    }
  );
  ...
  return (
    <div className="bg-white rounded-[2.25rem] p-4 shadow-sm border border-slate-200 flex flex-col">
      ...
      <form onSubmit={form.handleSubmit} className="flex flex-col gap-2.5">
        ...
        <button
          type="submit"
          disabled={!accountId || !selectedCategoryId || form.isSubmitting}
